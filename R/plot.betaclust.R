                           ...
                           plot_type="ggplot",title=NULL,scale_param="free_y")
{
  ...
  if(what == "density")
  {
    ...
    }else
    {
      ...
      f_labels<-data.frame(Cluster=seq(1,length(object$optimal_model_results$cluster_size),by=1),label=as.vector(round((object$optimal_model_results$cluster_size/object$C),4)))
