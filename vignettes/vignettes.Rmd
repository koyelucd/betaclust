...
author: "Koyel Majumdar, Andrea Rau, Florence Jaffrezic, Thomas Brendan Murphy and Isobel Claire Gormley"
...
The area-uner-curve (AUC) and the Wasserstein distance (WD) methods are employeed to find the level of dissimilarity between the *R=2* cumulative distributions estimated for each of the *K=9* clusters. As there are only 2 sample types for differential methylation analysis only a single dissimilarity is obtained for each of the 9 clusters.

We then employed a threshold of 0.65 for the AUC metric and/or a threshold of 0.06 for the WD metric for identifying the clusters
1,2 and 3 as the most differentially methylated clusters.
